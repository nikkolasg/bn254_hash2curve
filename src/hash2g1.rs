...
use digest::generic_array::{typenum::U32, typenum::U48};
...
impl FromOkm<L> for Fq {
    fn from_okm(data: &[u8; L]) -> Self {
        ...
        .unwrap();
        println!(" - Setting Bytes = {}", hex::encode(&data[..]));
        let mut x = BigUint::from_bytes_be(&data[..]);
        x = x.mod_floor(&p);
        println!(" - Gives back BigUint = {}", x.to_str_radix(10));
        let el = Fq::from(x);
        println!(" - Gives back Element = {}", el.to_string());
        el
    }
}
...
pub trait Hash2FieldBN254 {
    fn hash_to_field(msg: &[u8], dst: &[u8], count: usize) -> Vec<Self>
    where
        Self: Sized;
}
...
impl Hash2FieldBN254 for Fq {
    fn hash_to_field(msg: &[u8], dst: &[u8], count: usize) -> Vec<Fq> {
        ...
        println!("pseudo random = {}", hex::encode(&pseudo_random_bytes));
...
pub fn MapToCurve1(u: Fq) -> G1 {
    ...
    let mut z: Fq = Fq::from_str(
        "6350874878119819312338956282401532409788428879151445726012394534686998597021",
    )
    .unwrap();
    let mut c1: Fq = Fq::from_str(
        "3515256640640002027109419384348854550457404359307959241360540244102768179501",
    )
    .unwrap();
    let mut c2: Fq = Fq::from_str(
        "7768683996859727954953724731427871339453941139073188968338321679979113805781",
    )
    .unwrap();
    let mut c3: Fq = Fq::from_str(
        "5174556184976127869173189452163337195348491024958816448391141365979064675186",
    )
    .unwrap();
    let mut c4: Fq = Fq::from_str(
        "2609072103093089037936242735953952295622231240021995565748958972744717830193",
    )
    .unwrap();
...
mod tests {
    ...
    fn hash2field_test() {
        ...
        assert!(
            u[0] == Fq::from_str(
                "7951370986911800256774597109927097176311261202951929331835478768207980370345"
            )
            .unwrap()
        );
        assert!(
            u[1] == Fq::from_str(
                "8293556689416303717881563281438712057465092967957999993252567763605862533321"
            )
            .unwrap()
        );
        ...
        assert!(
            u[0] == Fq::from_str(
                "21473511429296129787161665655193361189518945362859158450118183976151186446397"
            )
            .unwrap()
        );
        assert!(
            u[1] == Fq::from_str(
                "17399580852346357386985693124899680967448413221719274165687915620563859110222"
            )
            .unwrap()
        );
        ...
        assert!(
            u[0] == Fq::from_str(
                "2044513137826275527915612741016000753813717898656440700304636055936191489587"
            )
            .unwrap()
        );
        assert!(
            u[1] == Fq::from_str(
                "11602613730878338430727365363851039884306398846852682736694594518413917134846"
            )
            .unwrap()
        );
    }
    ...
    fn MapToCurve1_test() {
        ...
        assert!(
            q0 == G1::new(
                Fq::from_str(
                    "9192524283969255398734814822241735402343760142215332184598869386265143635853"
                )
                .unwrap(),
                Fq::from_str(
                    "14750013374492649779039522357455217122947104756064249167130349093550158884161"
                )
                .unwrap()
            )
        );
        assert!(
            q1 == G1::new(
                Fq::from_str(
                    "2219529064992744478098731193326567804904209297389738932911685687632211367327"
                )
                .unwrap(),
                Fq::from_str(
                    "1910726159786414357764375718946103460897900837832114831609513656424867805207"
                )
                .unwrap()
            )
        );
        ...
        assert!(
            q0 == G1::new(
                Fq::from_str(
                    "18460180777384996805517037410124907200489198402642233028065858702876325100173"
                )
                .unwrap(),
                Fq::from_str(
                    "7297925201307108404837100086863759533322513325723985709501528779399363778017"
                )
                .unwrap()
            )
        );
        assert!(
            q1 == G1::new(
                Fq::from_str(
                    "3555154583542724794659651262588560064541528505277497563560719769602741821875"
                )
                .unwrap(),
                Fq::from_str(
                    "16977637197741440727690443467244845071598833410411827382713029829487302630942"
                )
                .unwrap()
            )
        );
        ...
        assert!(
            q0 == G1::new(
                Fq::from_str(
                    "15648482829240231450830106706414350609765304380572340182587624553168656871227"
                )
                .unwrap(),
                Fq::from_str(
                    "2884090034870953736753092279678539049499634063513478774348194493913603274393"
                )
                .unwrap()
            )
        );
        assert!(
            q1 == G1::new(
                Fq::from_str(
                    "18124086957708989159022363715051003299508967919164499453496316230748020813590"
                )
                .unwrap(),
                Fq::from_str(
                    "16826684847259413750643448296038340935081016395629901184485922600208379683908"
                )
                .unwrap()
            )
        );
        ...
        assert!(
            u[0] == Fq::from_str(
                "19139799307876008157674469077244497844490197231122854489816996874209678928507"
            )
            .unwrap()
        );
        assert!(
            u[1] == Fq::from_str(
                "16149156964295957170548772524136742336424608142546544142472739268994996707563"
            )
            .unwrap()
        );
        let q0 = MapToCurve1(u[0]);
        ...
        assert!(
            q0 == G1::new(
                Fq::from_str(
                    "12812625340294489398993548898688649895606244530534785322892890577243153100708"
                )
                .unwrap(),
                Fq::from_str(
                    "14430750872577414903993343696062677117017041221676892932403418483169263778689"
                )
                .unwrap()
            )
        );
        assert!(
            q1 == G1::new(
                Fq::from_str(
                    "15057612003824249181576746168110806738223995458659553230425471086211724164874"
                )
                .unwrap(),
                Fq::from_str(
                    "7086679767009137399570643369757025464320023320148085000688641996630730281917"
                )
                .unwrap()
            )
        );
    ...
    fn HashToG1_test() {
        ...
        assert!(
            q == G1::new(
                Fq::from_str(
                    "16267524812466668166267883771992486438338357688076900798565538061554532963281"
                )
                .unwrap(),
                Fq::from_str(
                    "1844916233815282837483764409618609279507070495361570126601873459268232811805"
                )
                .unwrap()
            )
        );
        ...
        assert!(
            q == G1::new(
                Fq::from_str(
                    "11077683243901808951859264683654586764079462418577485658911541848692394044746"
                )
                .unwrap(),
                Fq::from_str(
                    "4858124309270455482359664916577923636817363175462672327824733704859450489677"
                )
                .unwrap()
            )
        );
        ...
        assert!(
            q == G1::new(
                Fq::from_str(
                    "449076125358095157945547407089359408531318284903480972761046551095956160348"
                )
                .unwrap(),
                Fq::from_str(
                    "3427911873443593747709927415036866402371639925174562008506349359915732032632"
                )
                .unwrap()
            )
        );
        ...
        assert!(
            q == G1::new(
                Fq::from_str(
                    "4790658965958450548702669593570794336562317867247372723806336874591549759110"
                )
                .unwrap(),
                Fq::from_str(
                    "1163238807669877429342450210709044731909255047583162173012265677391336920021"
                )
                .unwrap()
            )
        );
        ...
        assert!(
            q == G1::new(
                Fq::from_str(
                    "763925112321939766609678334678065587309331741428777416269918389033192485838"
                )
                .unwrap(),
                Fq::from_str(
                    "12636771015364464547273606234110225240317241569495907283228710706019336772016"
                )
                .unwrap()
            )
        );
